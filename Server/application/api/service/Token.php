...
class Token//更多的内容是关于权限的判断
{
    ...
    public static function generateToken()
    {
        ...
        $tokenSalt = config('setting.token_salt');//获取后缀
    ...
    public static function needPrimaryScope()
    {
        ...
        if ($scope) {//获取到的SCOPE有可能是空的，所以要判断
    ...
    public static function getCurrentTokenVar($key)
    {
        $token = Request::instance()
            ->header('token');//据说Token都放在http请求中
        $vars = Cache::get($token);//通过令牌从缓存中获取
        if (!$vars)
        {
            throw new TokenException();
        }
        else {
            if(!is_array($vars))//获取得到的变量有很多，也有可能是一个比如uid，
                //所以有可能不是数组，那么就不用使用json序列化了
            {
